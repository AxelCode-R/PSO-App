...
shinyServer(function(input, output) {
  ...
  output$plot_3D <- renderPlotly({
    ...
    isolate({
      ...
      space <- upper-lower
      max_iter <- input$iter
      fast_plot <- input$fast_plot
    })


    if(fast_plot){

      showNotification("Der schnelle Plot ohne transitions wurde ausgewählt. Die Generierung sollte schnell fertig sein.", duration=10, type = "message")
      ...
      showNotification("Der langsamere Plot mit transitions wurde ausgewählt. Die Generierung kann ein wenig dauern.", duration=10, type = "message")
      ...
      fig <- plot_ly() %>%
        ...
        add_trace(
          ...
          type = 'scatter3d',
          showlegend=F,
          marker = list(color = 'red', size=~if_else(opacity==1, 18, 10), opacity = ~opacity, showscale = F, line=list(width=0))
        ) %>%
        add_trace(
          data = X_anchors,
          x=~axis_1,
